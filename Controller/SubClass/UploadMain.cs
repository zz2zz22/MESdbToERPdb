...
namespace MESdbToERPdb
{
    public class UploadMain
    {
        ...
        public void GetListTransferOrder(string dIn, string dOut)
        {
            ...
            {
                SystemLog.Output(SystemLog.MSG_TYPE.War, "Missing data D2:", missingUUIDs.Length.ToString() + "data line!");
                for (int i = 0; i < missingUUIDs.Length; i++)
                {
                    try
                    {
                        string jobmId = missingUUIDs[i];
                        ...
                        sqlGetTable.Append("operation_uuid, operation_no, operation_name, product_uuid, product_no ,product_lot_no, move_out_qty, move_out_pass_qty, move_out_failed_qty, move_out_date, create_by, update_by, create_date, update_date");
                        sqlGetTable.Append(" from job_move ");
                        sqlGetTable.Append(" where uuid = '" + missingUUIDs[i] + "'");
                        data.sqlDataAdapterFillDatatable(sqlGetTable.ToString(), ref temp);

                        SystemLog.Output(SystemLog.MSG_TYPE.Err, "Đơn chuyển", temp.Rows[i]["move_no"].ToString() + "(" + jobmId + ").");

                        sqlMESInterCon con2 = new sqlMESInterCon();
                        string jobOrder_id = temp.Rows[i]["job_order_uuid"].ToString();
                        string productLotNo = temp.Rows[i]["product_lot_no"].ToString();

                        int checkD1orD2 = CheckProcess(temp.Rows[i]["work_order_process_uuid"].ToString(), temp.Rows[i]["operation_uuid"].ToString());
                        int checkD1orD2_D511 = CheckProcessD511(temp.Rows[i]["work_order_process_uuid"].ToString(), temp.Rows[i]["operation_uuid"].ToString());
                        bool checkProcessJC = CheckProductCodeForProcessD511(temp.Rows[i]["work_order_process_uuid"].ToString(), temp.Rows[i]["product_no"].ToString());
                        if (checkD1orD2 != 3) //check công đoạn sau MQC thì không lấy dữ liệu
                        {
                            string DateUp = DateTime.Now.ToString();
                            string TimeUp = DateTime.Now.ToString("HH:mm:ss");
                            ...
                            int jmMOPassQty = int.Parse(temp.Rows[i]["move_out_qty"].ToString());
                            ...
                            TimeSpan minDate = new TimeSpan(1, 0, 0, 0);
                            int flag = 0;
                            for (int a = 0; a < dtJobRecord.Rows.Count; a++)
                            {
                                TimeSpan subDate = new TimeSpan(0, 0, 0, 0);
                                subDate = Convert.ToDateTime(dtJobRecord.Rows[a]["create_date"].ToString()).Subtract(createDateJM);
                                if (subDate < minDate)
                                {
                                    minDate = subDate;
                                    flag = i;
                            ...
                            string operationCode = temp.Rows[i]["operation_no"].ToString();
                            string ParentOrgCode = GetParentOrganizationCode(temp.Rows[i]["belong_organization"].ToString());
                            bool isOperationJM = false;
                            if (temp.Rows[i]["operation_no"].ToString().Substring(0, 2) == "JM")
                            {
                                isOperationJM = true;
                            }
                            else isOperationJM = false;
                            ...
                            if (productionCodeHeaders != null)
                            {
                                for (int b = 0; b < productionCodeHeaders.Length; b++)
                                {
                                    if (MP == productionCodeHeaders[b])
                                    {
                                        checkIsProductionCode = true;
                                    }
                                }
                            ...
                            if (erpCode != "")
                            {
                                ...
                                DateTime transDate = Convert.ToDateTime(temp.Rows[i]["move_out_date"].ToString());
                                insertERP_D201 classinsertD2 = new insertERP_D201();
                                insertERP_D101 classinsertD1 = new insertERP_D101();

                                if (checkIsProductionCode) //check là mã có thuộc ( A511, B511, P511, J511, P512 ) hay kg
                                {
                                    if (MP == "P512" && temp.Rows[i]["belong_organization"].ToString() == "3VFVREIJ1R41") //Nếu là P512 thì xét phải có phải là công đoạn có mã JM của bộ phận J01-2 thì không phát sinh phiếu
                                    {
                                        if (!isOperationJM)
                                        {
                                            if (OKQty + NGQty > 0)
                                            {
                                                if (!checkSemiOngLon.Contains(checkSemiValue))
                                                {
                                                    if (checkD1orD2 == 2)
                                                    {
                                                        ...
                                                        {
                                                            //update D201 to realtime
                                                            classinsertD2.InsertdataToERP_D201(MP, SP, ParentOrgCode, OKQty, NGQty, RWQty, transDate, DateUp, TimeUp, timeIn, timeOut, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString());
                                                            classinsertD2.updateERPD201(MP, SP, OKQty, NGQty, RWQty, DateUp, TimeUp, transDate); //check transdate 
                                                        }
                                                        else
                                                        {
                                                            SystemLog.Output(SystemLog.MSG_TYPE.Nor, "GetListTransferOrderD2", "Không có phiếu chuyển do MOCTA011 = Y/y.");
                                                            DataReport.addReport(DataReport.RP_TYPE.Fail, DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"), transDate.ToString("dd/MM/yyyy HH:mm:ss"), "", "", "", MP + SP, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString(), OKQty.ToString(), NGQty.ToString(), reworkQ, "", "Không có phiếu chuyển do MOCTA011 = Y/y.");
                                                        }
                                                    }
                                                    else
                                                    {
                                                ...
                                                {
                                                    SystemLog.Output(SystemLog.MSG_TYPE.Nor, "GetListTransferOrderD2", "Không có phiếu chuyển D201 do lệnh sản xuất có chứa 'SEMI'");
                                                    DataReport.addReport(DataReport.RP_TYPE.Error, DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"), transDate.ToString("dd/MM/yyyy HH:mm:ss"), "", "", "", MP + SP, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString(), OKQty.ToString(), NGQty.ToString(), reworkQ, "", "Không có phiếu chuyển D201 do lệnh sản xuất có chứa 'SEMI'");
                                                }
                                            }
                                            else
                                            {
                                                SystemLog.Output(SystemLog.MSG_TYPE.Nor, "GetListTransferOrderD2", "Không thể tạo phiếu chuyển do 'Số lượng hoàn thành(OK)' = 0 và 'Số lượng phế(NG)' = 0 !");
                                                DataReport.addReport(DataReport.RP_TYPE.Error, DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"), transDate.ToString("dd/MM/yyyy HH:mm:ss"), "", "", "", MP + SP, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString(), OKQty.ToString(), NGQty.ToString(), reworkQ, "", "Không thể tạo phiếu chuyển do 'Số lượng hoàn thành(OK)' = 0 và 'Số lượng phế(NG)' = 0 !");
                                            }
                                        }
                                        else
                                        {
                                            SystemLog.Output(SystemLog.MSG_TYPE.Nor, "GetListTransferOrderD2", "Không có phiếu chuyển D201 do công đoạn thuộc J01-2 và có mã công đoạn là JM.");
                                            DataReport.addReport(DataReport.RP_TYPE.Error, DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"), transDate.ToString("dd/MM/yyyy HH:mm:ss"), "", "", "", MP + SP, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString(), OKQty.ToString(), NGQty.ToString(), reworkQ, "", "Không có phiếu chuyển D201 do công đoạn thuộc J01-2 và có mã công đoạn là JM.");
                                        }
                                    }
                                    else
                                    {
                    ...
                    {
                        SystemLog.Output(SystemLog.MSG_TYPE.Nor, "GetListTransferOrderD2", ex.Message);
                        DataReport.addReport(DataReport.RP_TYPE.Error, DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"), "", "", "", "", "", temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString(), "", "", "", "", ex.Message);
                    }
                }
            }
        }
        public void GetListTransferOrderD1(string dIn, string dOut)
        {
            ...
            if (missingUUIDs.Length > 0)
            {
                SystemLog.Output(SystemLog.MSG_TYPE.War, "Missing data D1:", missingUUIDs.Length.ToString() + "data line!");
                for (int i = 0; i < missingUUIDs.Length; i++)
                {
                    try
                    {
                        ...
                        int checkD1orD2 = CheckProcess(temp.Rows[i]["work_order_process_uuid"].ToString(), temp.Rows[i]["operation_uuid"].ToString());
                        int checkD1orD2_B511 = CheckProcessB511(temp.Rows[i]["work_order_process_uuid"].ToString(), temp.Rows[i]["operation_uuid"].ToString());
                        int checkD1orD2_D511 = CheckProcessD511(temp.Rows[i]["work_order_process_uuid"].ToString(), temp.Rows[i]["operation_uuid"].ToString());
                        bool checkProcessJC = CheckProductCodeForProcessD511(temp.Rows[i]["work_order_process_uuid"].ToString(), temp.Rows[i]["product_no"].ToString());
                        if (checkD1orD2 != 3) //check công đoạn sau MQC thì không lấy dữ liệu
                        {
                            ...
                            if (erpCode != "")
                            {
                                ...
                                if (checkIsProductionCode) //check là mã có thuộc ( A511, B511, P511, J511, P512 ) hay kg
                                {
                                    if (MP == "P512" && temp.Rows[i]["belong_organization"].ToString() == "3VFVREIJ1R41") //Nếu là P512 thì xét phải có phải là công đoạn có mã JM của bộ phận J01-2 thì không phát sinh phiếu
                                    {
                                            ...
                                            if (OKQty + NGQty > 0)
                                            {
                                                if (!checkSemiOngLon.Contains(checkSemiValue))
                                                {
                                                    if (checkD1orD2 == 1)
                                                    {
                                                        if (isFinished == false)
                                                        {
                                                            classinsertD1.InsertdataToERP_D101(MP, SP, ParentOrgCode, OKQty, transDate, DateUp, TimeUp, timeIn, timeOut, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString());
                                                            classinsertD1.updateERPD101(MP, SP, OKQty, DateUp, TimeUp, transDate);
                                                        }
                                                        else
                                                        {
                                                ...
                                                {
                                                    SystemLog.Output(SystemLog.MSG_TYPE.Nor, "GetListTransferOrderD1", "Không có phiếu chuyển D101 do lệnh sản xuất có chứa 'SEMI'");
                                                    DataReport.addReport(DataReport.RP_TYPE.Error, DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"), transDate.ToString("dd/MM/yyyy HH:mm:ss"), "", "", "", MP + SP, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString(), OKQty.ToString(), NGQty.ToString(), reworkQ, "", "Không có phiếu chuyển D101 do lệnh sản xuất có chứa 'SEMI'");
                                                }
                                            }
                                            else
                                            {
                                        ...
                                        {
                                            SystemLog.Output(SystemLog.MSG_TYPE.Nor, "GetListTransferOrderD1", "Không có phiếu chuyển D101 do công đoạn thuộc J01-2 và có mã công đoạn là JM.");
                                            DataReport.addReport(DataReport.RP_TYPE.Error, DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"), transDate.ToString("dd/MM/yyyy HH:mm:ss"), "", "", "", MP + SP, temp.Rows[i]["move_no"].ToString(), temp.Rows[i]["product_no"].ToString(), temp.Rows[i]["operation_no"].ToString(), temp.Rows[i]["operation_name"].ToString(), OKQty.ToString(), NGQty.ToString(), reworkQ, "", "Không có phiếu chuyển D101 do công đoạn thuộc J01-2 và có mã công đoạn là JM.");
                                        }
                                    }
                                    else
                                    {
